...
export const appRouter = router({
  ...
  auth: router({
    me: publicProcedure.query(opts => opts.ctx.user ? ({
      ...
      valorMensalidade: opts.ctx.empresa?.valorMensalidade ?? 0,
    ...
    login: publicProcedure.input(z.object({
      ...
      return {
        user: {
          ...
          valorMensalidade: empresa?.valorMensalidade ?? 0,
  ...
  empresas: router({
    ...
    atualizar: platformAdminProcedure.input(z.object({
      ...
      valorMensalidade: z.number().int().min(0).max(100000).optional(),
